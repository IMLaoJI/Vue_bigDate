...
window.$$ = (function() {
    ...
    $$.request = function(url, data, success, error, options) {
        ...
        options = $.extend({
            ...
            complete: function() {
                if (options.loading) layer.close(index);
            }
        });

        if (options.loading) index = layer.load(2);
        return $.ajax(options);
    };


    $$.loadForm = function(target, data) {
        var form = $(target);
        for (var name in data) {
            ...
            form.find('.form-control-static[data-name="' + name + '"]').text(val);
        }
